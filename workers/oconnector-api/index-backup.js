/**
 * oConnector API Worker - Versão Completa com Autenticação
 * Deploy: Cloudflare Workers
 * Database: D1 (oconnector_db)
 */
...
/**
 * Importar handler de IA
 * Nota: Imports devem estar no topo, mas Cloudflare Workers suporta imports dinâmicos
 */
async function loadAIChatHandler() {
  // Em Cloudflare Workers, podemos usar import dinâmico ou import estático no topo
  // Para compatibilidade, vamos definir inline ou usar import no topo
  // Por enquanto, vamos implementar inline
  return null; // Será implementado inline
}

/**
 * Handler de IA inline (alternativa ao import)
 */
  ...
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/whatsapp/')) {
      const { 
        ...
        handleSendMessage, 
        handleGetStatus 
      } = await import('./whatsapp-handlers.js');
