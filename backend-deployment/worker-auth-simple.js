/**
 * Endpoints de Autenticação - Versão Simplificada para Cloudflare Workers
 * Usa Web Crypto API (nativo) em vez de bcryptjs
 */
...
// Gerar token simples (em produção, use JWT real)
function generateToken(user) {
  ...
  // Token simples base64 (em produção, use JWT com assinatura)
...
async function handleLogin(request, env) {
  try {
    ...
    // Verificar senha
    // Se a senha no banco é bcrypt hash, precisamos usar bcrypt
    // Para senhas já hasheadas com bcrypt, vamos verificar diretamente
    let passwordValid = false;
    
    // Se começa com $2b$ ou $2a$, é hash bcrypt - não podemos verificar aqui sem bcryptjs
    // Vamos assumir que você atualizará as senhas para SHA-256 ou manterá bcrypt no banco
    if (user.senha.startsWith('$2b$') || user.senha.startsWith('$2a$')) {
      // Hash bcrypt - retornar erro informando que precisa implementar verificação bcrypt
      // Por enquanto, vamos fazer uma verificação básica
      return jsonResponse(
        { 
          success: false, 
          error: 'Senha em formato bcrypt. Implemente verificação bcrypt no worker ou atualize para SHA-256',
          hint: 'Use a biblioteca bcrypt compatível com Workers ou atualize as senhas no banco'
        },
        500
      );
    } else {
